...
; R2 is store hex number of a/any letter
; R3 is store bit mask
; perform AND operation R2, R3

LOOP_TO_UPPER
    ; 1. get hex/binary number of a/any letter
    LDR R2, R1, #0
    ; 2. load mask to R3
    LD R3, MASK
    ; 3. perform AND operation in R2, R3
    AND, R2, R3, R2
    ; 4. store back R2 to address R1
    STR R2, R1, #0
    ; 5. increment R1, and decrement R5
    ADD R1, R1, #1
    ADD R5, R5, #-1
    ; 6. branch if positove, repeat step 1 to 5
    BRp LOOP_TO_UPPER
END_LOOP_TO_UPPER
